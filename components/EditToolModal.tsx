...
const EditToolModal: React.FC<EditToolModalProps> = ({ tool, onClose, onEditTool }) => {
  ...
  const [toolUrl, setToolUrl] = useState(tool.url || '');
  const [faviconUrl, setFaviconUrl] = useState<string | null>(null);
  const [isDetecting, setIsDetecting] = useState(false);

  const handleFetchFavicon = async (url: string) => {
    if (!url || isDetecting) {
        if (!url) setFaviconUrl(null);
        return;
    };
  ...
  const renderIconPreview = () => {
    ...
    return <div className="text-gray-400 text-sm">파비콘을 찾을 수 없거나 URL이 유효하지 않습니다.</div>;
  };
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="text-xl font-bold mb-6">AI 서비스 수정</h2>
        
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label htmlFor="service-name-edit" className="block text-sm font-medium text-gray-700 mb-1">
              서비스 이름
            </label>
            <input
              type="text"
          ...
          <div>
            <label htmlFor="service-url-edit" className="block text-sm font-medium text-gray-700 mb-1">
              URL
            </label>
            <input
              type="text"
